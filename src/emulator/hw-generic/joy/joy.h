...
#define joy_reset_dev_state(joy_devid)     \
    {                                      \
        g_joy.dev[joy_devid].state = 0xff; \
    }

#define JOY_STATEBIT_RESET(state, bit) \
    {                                  \
        state &= ~(1 << bit);          \
    }
...
extern "C"
{
    ...
    extern uint8_t joy_read_byte(en_JOY_DEVID joy_devid);
#ifdef __cplusplus
}
#endif
