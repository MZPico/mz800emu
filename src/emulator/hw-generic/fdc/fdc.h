 ...
 * ## State
 *
 * Globální stav drží jediná struktura `g_fdc` typu `st_FDC`:
 ...
 * operují pouze nad `g_fdc.connected`.
 *
 * `FDC_TEST_DRIVE_ID_MOUNTED(drive_id)` expanduje na volání funkce
 * `fdc_test_drive_id_mounted(drive_id)`.
 *
 * License: GPLv3.
 */
...
#define FDC_NUM_DRIVES 4
     ...
     * vlastní WD279x chip a 4 mechaniky.
     *
    ...
    /** Test: FDC není připojený. */
#define FDC_TEST_NOT_CONNECTED (g_fdc.connected != FDC_CONNECTED)
    /** Test: FDC (WD279x) je připojený. */
#define FDC_TEST_WD279X_CONNECTED (g_fdc.connected == FDC_CONNECTED)

    /** Nastav stav na "nepřipojeno". */
#define FDC_SET_NOT_CONNECTED() (g_fdc.connected = FDC_DISCONNECTED)
    /** Nastav stav na "WD279x připojen". */
#define FDC_SET_WD279X_CONNECTED() (g_fdc.connected = FDC_CONNECTED)

    /**
     * @brief Test, zda je v dané mechanice nasazený DSK obraz.
     * @param drive_id index mechaniky (0..3)
     */
#define FDC_TEST_DRIVE_ID_MOUNTED(drive_id) (fdc_test_drive_id_mounted(drive_id))
     ...
     *
     * BUS read handler (port 0xD8..0xDF). Volá chip read a aplikuje inverze
     * D0..D7 podle `g_fdc.bus_xlate`.
     *
     ...
     *
     * BUS write handler (port 0xD8..0xDF). Aplikuje inverze D0..D7 vstupních
     ...
     * @param drive_id index mechaniky (0..3)
     */
    extern void fdc_ui_mount(unsigned drive_id);
     ...
     * stav do `g_fdc.drive[drive_id]`. Při remount nejdřív zavře předchozí
     ...
     */
    extern int fdc_test_drive_id_mounted(unsigned drive_id);

    /**
     * @brief Vrátí název cfgfile klíče DSK cesty pro danou mechaniku.
     *
     * Klíče: `wd279x_fdd0_dskpath` .. `wd279x_fdd3_dskpath`. Statický
     * buffer per drive_id.
     *
     * @param drive_id index mechaniky (0..3)
     ...
     */
    extern const char *fdc_dskpath_keyname(unsigned drive_id);
     ...
     */
    extern void fdc_cfg_set_dskpath(unsigned drive_id, const char *value);

    /**
     * @brief Vrátí název cfgfile klíče "readonly" pro mechaniku.
     *
     * Klíče: `wd279x_fdd0_readonly` .. `wd279x_fdd3_readonly` (BOOL).
     ...
     */
    extern const char *fdc_readonly_keyname(unsigned drive_id);
     ...
     */
    extern int fdc_cfg_get_readonly(unsigned drive_id);
     ...
     * @param value 0/1
     */
    extern void fdc_cfg_set_readonly(unsigned drive_id, int value);

    /**
     * @brief Vrátí název cfgfile klíče "storage_mode" pro mechaniku.
     *
     * Klíče: `wd279x_fdd0_storage_mode` .. `wd279x_fdd3_storage_mode` (TEXT).
     * Hodnoty: "cached" | "direct" | "discard". Default "cached".
     */
    extern const char *fdc_storage_mode_keyname(unsigned drive_id);
     ...
     */
    extern const char *fdc_cfg_get_storage_mode(unsigned drive_id);
     ...
     */
    extern void fdc_cfg_set_storage_mode(unsigned drive_id, const char *value);
     ...
     */
    extern int fdc_sync_drive(unsigned drive_id);
     ...
     */
    extern int fdc_drive_has_unsaved_changes(unsigned drive_id);
     ...
     * @return 1 = ano, 0 = ne.
     */
    extern int fdc_drive_fs_readonly(unsigned drive_id);
     ...
     * @return 1 = ano, 0 = ne.
     */
    extern int fdc_drive_has_ram_changes(unsigned drive_id);
     ...
     * @return 1 = OK, 0 = chyba.
     */
    extern int fdc_drive_force_save_to_file(unsigned drive_id);
