 ...
 *
 * @note Importováno do mz800new z ~/projects/mzdisk/src/mzdisk/libs/mzglyphs/
 *       dne 2026-05-24 pro feature Memory Browser (debugger).
 *
...
/**
 * @brief Vrátí Unicode PUA codepoint pro daný video kód a charset.
 *
 * Mapuje Sharp MZ video kód na codepoint v Unicode Private Use Area
 * podle vzorce: codepoint = 0xE100 + (charset * 0x100) + video_code.
 *
 * Výsledný codepoint vždy spadá do rozsahu U+E100-U+E4FF, kde má
 * font mzglyphs.ttf namapovány glyfy pro 4 charsety (EU1/EU2/JP1/JP2).
 *
 * Použití: callers které potřebují přímo numerický codepoint (např.
 * ImGui glyph lookup), bez nutnosti UTF-8 enkódování.
 *
 * @param video_code Sharp MZ video kód (0x00-0xFF).
 * @param charset    Znaková sada fontu.
 * @return Codepoint v rozsahu U+E100-U+E4FF.
 *
 * @note Pro UTF-8 řetězec použít mzglyphs_to_utf8() nebo mzglyphs_to_utf8_buf().
 */
extern uint32_t mzglyphs_get_codepoint ( uint8_t video_code, en_MZGLYPHS_CHARSET charset );
