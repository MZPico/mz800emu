 ...
 */
static int symtab_bsearch(const z80_symtab_t *tab, uint16_t addr, int *found)
{
    int lo = 0;
    int hi = tab->count - 1;
    ...
    *found = 0;
    ...
    while (lo <= hi) {
        int mid = lo + (hi - lo) / 2;
        uint16_t mid_addr = tab->entries[mid].addr;
 ...
 * @pre addr != NULL
 */
static int find_target_addr(const z80_dasm_inst_t *inst, uint16_t *addr)
{
    ...
    if (inst->flow == Z80_FLOW_RST) {
        if (inst->op1.type == Z80_OP_RST_VEC) {
            *addr = (uint16_t)inst->op1.val.imm8;
            return 1;
        }
        return 0;
    }
 ...
 * @pre addr != NULL
 */
static int find_mem_addr(const z80_dasm_inst_t *inst, uint16_t *addr)
{
    const z80_operand_t *ops[2] = { &inst->op1, &inst->op2 };
    for (int i = 0; i < 2; i++) {
