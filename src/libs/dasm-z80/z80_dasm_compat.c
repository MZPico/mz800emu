 ...
 */
static uint8_t readbyte_adapter(uint16_t addr, void *user_data)
{
 ...
 * @pre buf_size >= 1
 */
static int format_byte_dec(char *buf, int buf_size, uint8_t val)
{
 ...
 * @pre buf_size >= 1
 */
static int format_word_dec(char *buf, int buf_size, uint16_t val)
{
    return snprintf(buf, (size_t)buf_size, "%u", val);
}
                                ...
                                unsigned flags)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        case Z80_OP_REL8:
             ...
             */
            if (words_dec) {
                uint16_t abs_addr = z80_rel_to_abs(inst->addr,
